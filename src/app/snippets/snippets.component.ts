...
import { EditsnippetsComponent } from '../editsnippets/editsnippets.component';

interface UserData {
  id:number;
  helperTextNode: string;
  description: string;
  audiences: String;
  conditions: string;
  template: string;
...
// Paginator reference to connect to the mat-paginator in the template
  ...
  displayedColumns: string[] = [ 'helperTextNode', 'description', 'audiences','template','conditions','actions']; // Define table column names
  dataSource = new MatTableDataSource<UserData>([
    { id:1, helperTextNode: 'accessLearningHUBSDCText', description: 'instruction for SDC Supervisor on How to complete Training in Learning Hub', audiences:'SDC' ,template:'NH,AN,PD,VP',conditions:'' },
    { id:1, helperTextNode: 'accessLearningHUBSDCText', description: 'instruction for SDC Supervisor on How to complete Training in Learning Hub', audiences:'SDC' ,template:'NH,AN,PD,VP',conditions:'' },
    { id:1, helperTextNode: 'accessLearningHUBSDCText', description: 'instruction for SDC Supervisor on How to complete Training in Learning Hub', audiences:'SDC' ,template:'NH,AN,PD,VP',conditions:'' },
    { id:1, helperTextNode: 'accessLearningHUBSDCText', description: 'instruction for SDC Supervisor on How to complete Training in Learning Hub', audiences:'SDC' ,template:'NH,AN,PD,VP',conditions:'' },
    { id:1, helperTextNode: 'accessLearningHUBSDCText', description: 'instruction for SDC Supervisor on How to complete Training in Learning Hub', audiences:'SDC' ,template:'NH,AN,PD,VP',conditions:'' },
...
onSubmit(userData: any): void {
  ...
  deleteRecord(element: UserData): void {
    const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      data: {
        ...
        message: `Are you sure you want to delete the record for ${element.helperTextNode}?`
      }
    });
  ...
  onRowClick(template: SnippetsComponent): void {
    const dialogRef = this.dialog.open(EditsnippetsComponent, {
